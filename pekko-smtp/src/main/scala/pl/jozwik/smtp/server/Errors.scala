...
object Errors {
  ...
  def domainNotResolved(address: MailAddress): String =
    s"""$REQUEST_ACTION_ABORTED 4.1.8 Domain of sender address $address does not resolve"""
  ...
  def serviceNotAvailable(hostName: String, timeoutSeconds: Long): String =
    s"$SERVICE_NOT_AVAILABLE 4.4.2 $hostName Read timeout $timeoutSeconds seconds."
